...
export default function TesteMultiplasInteligencias() {
  ...
  const fetchDados = async () => {
    try {
      ...
      try {
        const perguntasResponse = await apiService.getPerguntas();
  ...
  const finalizarTeste = async () => {
    try {
      setLoading(true);
      message.success('Teste concluído! (Funcionalidade de salvamento em desenvolvimento)');
  ...
  return (
    <AdminLayout>
      <div style={{ padding: '24px' }}>
        ...
        {!testeIniciado && !testeConcluido && (
          <Card style={{ maxWidth: 600, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', padding: '40px 20px' }}>
              <PlayCircleOutlined style={{ fontSize: '64px', color: '#1890ff', marginBottom: 24 }} />
              <Title level={2}>Teste de Múltiplas Inteligências</Title>
              <Text style={{ fontSize: '16px', color: '#666', marginBottom: 32, display: 'block' }}>
                Este teste irá avaliar suas diferentes inteligências. Responda com sinceridade para obter um resultado mais preciso.
              </Text>
              ...
              <div style={{ marginBottom: 32 }}>
                <Row gutter={[16, 16]}>
                  <Col span={8}>
                    <Card size="small">
                      <Text strong>{perguntas.length}</Text>
                      <br />
                      <Text>Perguntas</Text>
                    </Card>
                  </Col>
                  <Col span={8}>
                    <Card size="small">
                      <Text strong>{possibilidades.length}</Text>
                      <br />
                      <Text>Opções</Text>
                    </Card>
                  </Col>
